...
| Toast preview | Conforme | Les soumissions valides affichent un toast sans créer de session fictive. |
| Préparation API | Conforme | `VITE_API_BASE_URL` active les appels vers `/auth/login` et `/auth/register`. |
| Stockage de session | Conforme preview | Aucun stockage local de token ou de mot de passe dans le module auth. |
...
Le mode preview ne vérifie pas de vrais identifiants, ne crée pas de compte et ne persiste pas de session. Lorsque le backend sera prêt, `VITE_API_BASE_URL` devra être configurée et les réponses d’erreur serveur devront respecter le contrat attendu par `auth.api.ts`.
