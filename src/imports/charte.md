...
Construite autour de l'axe chromatique du logo primaire (rose magenta `#E04070`).

> Note technique : `#E04070` est un **rose magenta** (R=224 — dominant, B=112 — mi-hauteur, G=64 — très bas). Le round-trip OKLCH standard tend le rouge-sang aux luminances extrêmes, donc les ramps sont calées **en RGB sur le ratio chroma-tique** de `#E04070` (G≈0,29·R, B≈0,50·R) plutôt que sur un OKLCH pur qui perdreait le magenta. Les neutres restent OKLCH chaud (H=30) car leur chroma est si faible que l'approximation est invisible.

### 2.1 Couleur primaire — Rose magenta ybook

```
--y-primary-800:      #471423;  /* bordeaux-noir, ombre du magenta */
--y-primary-700:      #7F243F;  /* rose-bordeaux profond */
--y-primary-600:      #B23259;  /* rose-bordeaux, hover/active */
--y-primary-500:      #E04070;  /* primaire de marque (référence exacte) */
--y-primary-400:      #E94274;  /* rose franc (usage sombre) */
--y-primary-300:      #F24579;  /* rose lumineux (disabled-light / accents) */
--y-primary-200:      #FC789F;  /* rose pastel */
/* tints pour fonds clairs (halo très léger) */
--y-primary-tint-100: #FCEEF0;  /* halo rose à 8% de la marque */
--y-primary-tint-050: #FDF4F4;  /* halo rose à 5% */
```

- Référence de marque : `--y-primary-500` = `#E04070` exact. Aucune autre nuance n'est la couleur de la marque ; les autres sont des outils.
- `--y-primary-400` est la variante à utiliser sur fond sombre (plus lumineuse pour conserver le même signal sans basculer au rouge).
- La ramp ne glisse **jamais** vers le rouge sang (`#D80000`) ou le rouge-noir (`#5F0000`) : le ratio G/R et B/R de la marque est préservé à chaque niveau pour garder le magenta-rose.

### 2.2 Couleur secondaire — Bordeaux ybook

Construite en RGB sur le ratio de `#B84870` (R=184 — dominant, B=112 — 61%, G=72 — 39%). Plus brune que la primaire, jamais rouge-sang.

```
--y-secondary-700:    #572234;  /* décor sombre, brun-bordeaux */
--y-secondary-500:    #B84870;  /* secondaire de marque (référence exacte) */
--y-secondary-400:    #CA4F7A;  /* contour hover, links de nav */
--y-secondary-300:    #E15888;  /* pastel bordeaux-rose */
```
...
Hue 30 en OKLCH, chroma très faible (0.012). Évoque le papier tiède plutôt qu'un gris froid.
...
Couleurs calmes — chroma volontairement réduit pour ne pas rivaliser avec le rose primaire. Le rouge-error est un rose-rouge `#C13F4E` qui dialogue avec la marque ; on n'introduit pas un rouge sang étranger.

```
--y-success:  #2E8B57;  /* vert violadox, calme */
--y-warning:  #C8956A;  /* ambre chaud doux */
--y-error:    #C13F4E;  /* rose-rouge cohérent avec la marque */
--y-info:     #5D8AA8;  /* bleu pétrole */
```

Versions claires et sombres (cf. §3) adaptent ces sémantiques sans toucher à la teinte. Chaque état est **toujours couplé à une icône** (§4.4).
...
:root {
  ...
  --text-link:        var(--y-primary-600); /* #B23259 — hover+, 5.2:1 sur ivoire */
  --text-link-hover:  var(--y-primary-700);

  /* Actions */
  --action-bg:            var(--y-primary-500);     /* boutons primaires #E04070 */
  --action-bg-hover:      var(--y-primary-600);
  --action-bg-active:     var(--y-primary-700);
  --action-fg:            #FFFFFF;
  --action-secondary-bg:  transparent;
  --action-secondary-fg:  var(--y-primary-700);
  --action-secondary-border: var(--y-primary-700); /* 1px */
  ...
  /* États sémantiques (clair).
     Fgs directement liés aux hex §2.4 ; bgs en OKLCH très faible chroma (5%) — halos teintés,
    jamais des fonds saturés qui crieraient à côté du rose magenta de marque. */
  --state-success-fg:  var(--y-success);  /* #2E8B57 */
  --state-success-bg:  oklch(0.94 0.03 145deg);
  --state-warning-fg:  oklch(0.40 0.10 75deg);   /* ambre assomi, AA sur ivoire */
  --state-warning-bg:  oklch(0.94 0.03 75deg);
  --state-error-fg:    var(--y-error);  /* #C13F4E */
  --state-error-bg:    oklch(0.94 0.03 16deg);    /* halo rose léger */
  --state-info-fg:     var(--y-info);  /* #5D8AA8 */
  --state-info-bg:     oklch(0.94 0.02 240deg);
  ...
  --shadow-primary: 0 8px 24px oklch(0.62 0.20 16deg / 0.32),
                     0 2px 6px oklch(0.62 0.20 16deg / 0.18);
}
```
...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --text-link:       var(--y-primary-400);     /* #E94274, 4.5:1 sur dark */
    --text-link-hover: var(--y-primary-300);

    /* Actions — le rose primary monte en L pour rester visible, sans devenir rouge néon */
    --action-bg:           var(--y-primary-400);  /* #E94274, plus lumineux qu'en light */
    --action-bg-hover:     var(--y-primary-300);
    --action-bg-active:    var(--y-primary-500);
    --action-fg:           oklch(0.14 0.012 30deg); /* encre chaude, contraste ~7:1 sur rose clair */
    --action-secondary-bg: transparent;
    --action-secondary-fg: var(--y-primary-300);
    --action-secondary-border: var(--y-secondary-400);
    ...
    /* États sémantiques (sombre).
       Fgs clairs et peu saturés pour not ressortir en rouge néon ; bgs profonds en OKLCH bas chroma. */
    --state-success-fg:  oklch(0.76 0.09 145deg);  /* vert pâle chaud */
    --state-success-bg:  oklch(0.26 0.04 145deg);
    --state-warning-fg:  oklch(0.78 0.09  75deg);  /* ambre chaud clair */
    --state-warning-bg:  oklch(0.26 0.04  75deg);
    --state-error-fg:    oklch(0.76 0.13  16deg);  /* rose clair, cohérent avec la marque */
    --state-error-bg:    oklch(0.28 0.05  16deg);
    --state-info-fg:     oklch(0.76 0.08 240deg);
    --state-info-bg:     oklch(0.26 0.03 240deg);
    ...
    --shadow-primary: 0 8px 24px oklch(0.70 0.16 16deg / 0.40),
                      0 2px 6px oklch(0.70 0.16 16deg / 0.24);
  }
}
```
...
| `--text-link` / `--canvas`            | ≥ 4.5:1     | `--y-primary-600` `#B23259` → 5.2:1 ✓      |
| `--text-primary` / `--canvas` (dark)  | ≥ 7:1       | papier calque L=.96 / encre L=.18 ≈ 13:1 ✓ |
| `--action-fg` / `--action-bg` (dark)   | ≥ 4.5:1     | encre chaude L=.14 / `--y-primary-400` `#E94274` ≈ 6.8:1 ✓ |
...
Sur fond rose clair (`--canvas-accent` `#FCEEF0`) → texte secondaire en `var(--y-primary-700)` `#7F243F` (rose-bordeaux foncé, ratio ~5:1 sur le tint).
Sur fond rose primaire (`--action-bg` `#E04070`) → texte secondaire en `oklch(0.82 0.04 16deg)` (rose pâle teinté de la même hue, ≥ 3:1 large-text).
Sur fond bordeaux (`--y-secondary-500`) → texte secondaire en `oklch(0.88 0.03 15deg)` (presque blanc teinté de la même hue).
...
| État     | Icône                 | Couleur `--state-*-fg` (clair / sombre)                  | Fond `--state-*-bg`    | Usage                  |
| -------- | --------------------- | -------------------------------------------------------- | --------------------- | ---------------------- |
| Succès   | check-mark            | vert `#2E8B57` / `oklch(.76 .09 145)`                   | halo vert clair 4%    | confirmation, completed |
| Avertissement | triangle          | ambre `oklch(.40 .10 75)` / `oklch(.78 .09 75)`          | halo ambre 3%         | validation, risque modéré |
| Erreur   | croix ou octogone     | rose-rouge `#C13F4E` / `oklch(.76 .13 16)`              | halo rose léger 3%    | errors de saisie, 404  |
| Info     | bulle « i »            | bleu `#5D8AA8` / `oklch(.76 .08 240)`                    | halo bleu 2%          | conseil, astuce         |
